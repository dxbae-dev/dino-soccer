...
export default class PlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.startText = this.add
      .text(width / 2, height / 2 - 50, "TAP TO START", {
        fontSize: "18px",
  ...
  activateFever() {
    ...
    this.tweens.addCounter({
      ...
      onComplete: () => {
        this.feverReq = Math.floor(this.feverReq * 1.2);
  ...
  update(time, delta) {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.cursors.right) && this.feverReady)
      this.activateFever();
    ...
    if (
      ...
      Math.floor(this.score) !== this.lastSavedScore &&
      this.score > 0
    ) {
      this.saveScore();
    ...
    this.currentSpeed = Math.max(
      -1200,
      Math.min(this.initialSpeed, calculatedSpeed),
    );

    this.player.setAnimationSpeed(
      Math.abs(this.currentSpeed) / Math.abs(this.initialSpeed),
    );

...
export default function GameUI({ onExit, children }) {
  ...
  useEffect(() => {
    ...
    const handleGameOver = async (payload) => {
      ...
      const isActuallyNewRecord =
        isNewRecord ||
        (numericFinalScore > currentHigh && numericFinalScore > 0);
      ...
      if (customMessage) {
        ...
        if (isActuallyNewRecord) {
          ...
          numericFinalScore >= currentHigh * 0.8 &&
          numericFinalScore > 0
        ) {
  ...
  const togglePause = () => {
    ...
    if (isPaused) {
      setCountdown((prev) => (prev !== null ? null : 3));
    } else {
      setIsPaused(true);
      EventBus.emit("toggle-pause", true);
    }
  ...
  if (scoreTier === "high")
    scoreColor = "text-amber-400 drop-shadow-[0_0_20px_rgba(251,191,36,0.8)]";
  else if (scoreTier === "low") scoreColor = "text-zinc-600";
  else if (scoreTier === "almost")
    scoreColor = "text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]";

  return (
    <div
      className="absolute inset-0 w-full h-full flex flex-col pointer-events-none font-sans"
        ...
        >
          <div className="absolute top-4 left-4 right-4 md:top-6 md:left-6 md:right-6 flex justify-between items-start z-40">
            <div className="flex flex-col gap-3 pointer-events-auto">
              ...
              <div className="flex gap-3 bg-black/40 px-4 py-2.5 rounded-full backdrop-blur-md border border-white/10 shadow-lg self-start">
                <div
                  className={`w-4 h-6 md:w-5 md:h-7 rounded-[4px] border-2 transition-all duration-300 ${lives <= 2 ? "bg-amber-400 border-amber-500 shadow-[0_0_15px_rgba(251,191,36,0.6)]" : "bg-transparent border-white/30"}`}
                />
                <div
                  className={`w-4 h-6 md:w-5 md:h-7 rounded-[4px] border-2 transition-all duration-300 ${lives <= 1 ? "bg-amber-400 border-amber-500 shadow-[0_0_15px_rgba(251,191,36,0.6)]" : "bg-transparent border-white/30"}`}
                />
              </div>
            </div>
            ...
            <div className="pointer-events-auto">
              ...
              >
                <img
                  ...
                  alt="Pause"
                  draggable="false"
                  className="w-5 h-5 brightness-0 invert group-active:scale-95 transition-transform pointer-events-none select-none"
                />
              </button>
            </div>
          </div>
              ...
              {activeCard === "fever" && (
                <div className="w-14 h-20 md:w-16 md:h-24 rounded-md shadow-[0_0_30px_rgba(6,182,212,0.8)] bg-cyan-500 border-2 border-cyan-400 animate-bounce flex items-center justify-center">
                  <img
                    ...
                    alt="Fever"
                    className="w-8 h-8 brightness-0 invert animate-pulse"
                  />
                </div>
              )}
            </div>
          ...
          <div className="absolute bottom-0 left-0 w-full h-28 md:hidden z-30 flex items-center justify-center gap-8 pointer-events-auto pb-4">
            <button
              onPointerDown={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "slide", true);
              }}
              onPointerUp={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "slide", false);
              }}
              onPointerLeave={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "slide", false);
            ...
            >
              <img
                ...
                alt="Slide"
                draggable="false"
                className="w-8 h-8 brightness-0 invert opacity-80 group-active:opacity-100 transition-opacity pointer-events-none select-none"
              />
            </button>
            ...
            <button
              onPointerDown={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "fever", true);
              }}
              onContextMenu={(e) => e.preventDefault()}
              style={{ WebkitTouchCallout: "none" }}
            ...
            >
              <img
                ...
                alt="Fever"
                draggable="false"
                className={`w-8 h-8 brightness-0 invert pointer-events-none select-none ${!(feverReady && !feverActive) && "opacity-30"}`}
              />
            </button>
            ...
            <button
              onPointerDown={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "jump", true);
              }}
              onPointerUp={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "jump", false);
              }}
              onPointerLeave={(e) => {
                e.preventDefault();
                EventBus.emit("virtual-input", "jump", false);
            ...
            >
              <img
                ...
                alt="Jump"
                draggable="false"
                className="w-8 h-8 brightness-0 invert opacity-90 group-active:opacity-100 transition-opacity pointer-events-none select-none"
              />
            </button>
          </div>
        </div>
      ...
      {(isPaused || isGameOver) && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-auto z-50">
          {countdown !== null ? (
            <div
              key={countdown}
              className="flex items-center justify-center animate-in zoom-in-50 duration-300 fade-in"
            >
              <span className="text-[120px] md:text-[180px] font-bold text-white tracking-widest drop-shadow-[0_0_30px_rgba(255,255,255,1)]">
                {countdown}
              </span>
            </div>
          ) : (
            <div className="flex flex-col items-center w-full max-w-sm p-8 animate-in fade-in duration-300">
                ...
                <div className="flex justify-between w-full mb-8 bg-black/40 px-6 py-4 rounded-2xl border border-white/10 shadow-lg">
                  ...
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <div className="flex gap-2">
                      <div
                        className={`w-3 h-5 rounded-[2px] border ${lives <= 2 ? "bg-amber-400 border-amber-500" : "bg-transparent border-white/30"}`}
                      />
                      <div
                        className={`w-3 h-5 rounded-[2px] border ${lives <= 1 ? "bg-amber-400 border-amber-500" : "bg-transparent border-white/30"}`}
                      />
                    </div>
                  </div>
                </div>
              ...
              {isGameOver && (
                <div className="flex flex-col items-center mb-10">
                  ...
                  <span className="text-xs md:text-sm text-zinc-400 tracking-widest uppercase mb-2 text-center leading-relaxed max-w-[250px]">
                    {gameOverMessage}
                  </span>
                  <span className={`text-5xl font-extralight ${scoreColor}`}>
                    {formattedScore}
                  </span>
                </div>
              )}
                ...
                {isGameOver ? (
                  <>
                    <button
                      ...
                      onContextMenu={(e) => e.preventDefault()}
                      style={{ WebkitTouchCallout: "none" }}
                      className="flex items-center justify-center gap-3 w-full py-4 rounded-full bg-white text-black font-medium tracking-widest uppercase text-xs hover:bg-zinc-200 transition-colors select-none"
                    >
                      <img
                        src="/assets/icons/refresh-ccw.svg"
                        alt="Retry"
                        draggable="false"
                        className="w-4 h-4 brightness-0 pointer-events-none"
                      />
                      RETRY
                    </button>
                    <button
                      ...
                      onContextMenu={(e) => e.preventDefault()}
                      style={{ WebkitTouchCallout: "none" }}
                      className="flex items-center justify-center gap-3 w-full py-4 rounded-full bg-white/5 border border-white/10 text-zinc-300 font-light tracking-widest uppercase text-xs hover:bg-white/10 hover:text-white transition-colors select-none"
                    >
                      <img

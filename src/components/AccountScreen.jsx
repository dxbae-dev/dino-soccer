...
import { getUserProfile, linkGoogleAccount, loginWithGoogle, generateTransferCode, recoverWithTransferCode } from "../firebase/userService";
...
export default function AccountScreen({ setScreen }) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-sm p-8 rounded-3xl bg-black/50 border border-white/10 backdrop-blur-xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <h2 className="text-xl font-light text-zinc-100 tracking-[0.2em] uppercase mb-6 text-center w-full border-b border-white/10 pb-4">
        Account
      </h2>
      ...
      <div className="w-full min-h-[220px] flex flex-col justify-center gap-4 mb-6">
        ...
        {view === "menu" && (
          <div className="flex flex-col gap-3">
            {!profile?.isLinked && (
              <button onClick={handleLinkGoogle} disabled={isLoading} className="w-full py-4 rounded-full bg-blue-600/90 text-white font-medium tracking-widest uppercase text-xs hover:bg-blue-500 transition-colors shadow-lg disabled:opacity-50">
                {isLoading ? "WAIT..." : "SECURE WITH GOOGLE"}
              </button>
            )}
            <button onClick={() => setView("code")} className="w-full py-4 rounded-full bg-black/60 border border-white/20 text-zinc-300 font-medium tracking-widest uppercase text-xs hover:bg-white/10 hover:text-white transition-colors">
              TRANSFER CODE
            </button>
            <div className="h-px bg-white/10 my-2 w-full"></div>
            <button onClick={() => setView("recover")} className="w-full py-4 rounded-full bg-amber-500/90 text-black font-medium tracking-widest uppercase text-xs hover:bg-amber-400 transition-colors shadow-lg">
              RECOVER ACCOUNT
            </button>
          </div>
        )}
        ...
        {view === "code" && (
          <div className="flex flex-col items-center text-center gap-4">
            ...
            {profile?.transferCode ? (
              ...
              <button onClick={handleGenerateCode} disabled={isLoading} className="w-full py-3 rounded-full bg-white/10 border border-white/20 text-white text-xs tracking-widest uppercase hover:bg-white/20 transition-colors disabled:opacity-50">
        ...
        {view === "recover" && (
          <div className="flex flex-col gap-4">
            <button onClick={handleRecoverGoogle} disabled={isLoading} className="w-full py-3 rounded-full bg-blue-600/90 text-white font-medium tracking-widest uppercase text-xs hover:bg-blue-500 transition-colors shadow-lg disabled:opacity-50">
            ...
            <form onSubmit={handleRecoverCode} className="flex flex-col gap-3">
              ...
              <button type="submit" disabled={isLoading} className="w-full py-3 rounded-full bg-amber-500/90 text-black font-medium tracking-widest uppercase text-xs hover:bg-amber-400 transition-colors disabled:opacity-50">
